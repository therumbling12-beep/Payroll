...
class ClaimGovernanceWorkflowService
{
    ...
    /**
     * Step 1: Immediate Supervisor Review
     */
    public function approveSupervisor(Claim $claim, ?float $approvedAmount = null, string $remarks = ''): Claim
    {
        return DB::transaction(function () use ($claim, $approvedAmount, $remarks) {
            ...
            $claim->supervisor_remarks = $remarks ?: 'Reviewed and endorsed by Immediate Supervisor.';
            $claim->approval_status = 'pending_hr';
            $claim->save();

            PayrollAuditTrail::create([
                'action' => 'CLAIM_SUPERVISOR_APPROVED',
                'model_type' => Claim::class,
                'model_id' => $claim->id,
                'user_name' => 'Immediate Supervisor',
                'ip_address' => request()->ip() ?? '127.0.0.1',
                'old_values' => ['approval_status' => 'pending'],
                'new_values' => [
                    'approval_status' => 'pending_hr',
                    'approved_amount' => (float) $claim->amount,
                    'remarks' => $claim->supervisor_remarks,
                ],
            ]);
    ...
    /**
     * Step 2: HR Management Validation
     */
    ...
    public function batchProcessClaims(array $claimIds, string $action, string $role, ?string $remarks = null): int
    {
        ...
        foreach ($claims as $claim) {
            match ($action) {
                'batch_approve_supervisor' => $this->approveSupervisor($claim, null, $remarks ?? 'Batch Supervisor Approved'),
