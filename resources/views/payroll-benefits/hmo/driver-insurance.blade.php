...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-black font-outfit text-gray-900 tracking-tight">TripWise Driver Accident Insurance Pool & Claims Governance</h1>
    ...
    <div x-data="{ 
        activeTab: 'claims', {{-- 'claims', 'ledger', 'policy', 'consultation' --}}
        ...
        <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-3 border-b border-gray-200 pb-3">
            ...
            <div class="flex flex-wrap items-center gap-2">
                <!-- Export Ledger CSV -->
                <a href="{{ route('hmo.driver-insurance.export-ledger') }}" 
        ...
        <div x-show="activeTab === 'claims'" x-transition class="space-y-6">
            ...
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 p-4 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <form action="{{ route('hmo.driver-insurance') }}" method="GET" class="flex flex-wrap items-center gap-3 flex-1">
                                    ...
                                    <td class="py-3.5 px-4 text-right">
                                        <div class="flex items-center justify-end gap-1.5">
                                            ...
                                            @if($claim->workflow_status === 'pending_hr')
                                                <button @click="openAction('hr', '{{ route('hmo.claim.approve-hr', $claim) }}', '{{ $claim->incident_number }}', '{{ $claim->bill_amount }}', 'Verified active trip logs and driver injury receipts.')" 
                                            ...
                                            @if($claim->workflow_status === 'pending_admin')
                                                <button @click="openAction('admin', '{{ route('hmo.claim.approve-admin', $claim) }}', '{{ $claim->incident_number }}', '{{ $claim->approved_amount ?: $claim->bill_amount }}', 'Vehicle damage report and incident assessment cleared.')" 
                                            ...
                                            @if($claim->workflow_status === 'pending_finance')
                                                <button @click="openAction('finance', '{{ route('hmo.claim.approve-finance', $claim) }}', '{{ $claim->incident_number }}', '{{ $claim->approved_amount ?: $claim->bill_amount }}', 'Disbursement approved and released from Driver Insurance Pool.')" 
                                            ...
                                            @if($claim->workflow_status === 'returned')
                                                <button @click="openAction('hr', '{{ route('hmo.claim.approve-hr', $claim) }}', '{{ $claim->incident_number }}', '{{ $claim->bill_amount }}', 'Re-evaluated following driver document submission.')" 
        ...
        <div x-show="activeTab === 'ledger'" x-transition class="space-y-6">
            <div class="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div class="p-5 border-b border-gray-100 flex items-center justify-between">
                    ...
                    <a href="{{ route('hmo.driver-insurance.export-ledger') }}" class="bg-gray-900 hover:bg-black text-white text-xs font-bold px-3.5 py-1.5 rounded-xl transition-all shadow-2xs">
                        Download CSV
                    </a>
                </div>
        ...
        <div x-show="showClaimModal" x-cloak class="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div @click.away="showClaimModal = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-xl p-6 space-y-4 max-h-[90vh] overflow-y-auto">
                ...
                <form action="{{ route('hmo.file-claim') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    @csrf
        ...
        <div x-show="showReturnModal" x-cloak class="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div @click.away="showReturnModal = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
                ...
                <template x-if="selectedClaim">
                    <form :action="'/hmo-benefits/driver-insurance/claim/' + selectedClaim.id + '/return'" method="POST" class="space-y-4">
                        @csrf
        ...
        <div x-show="showPolicyModal" x-cloak class="fixed inset-0 bg-black/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
            <div @click.away="showPolicyModal = false" class="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
                ...
                <form action="{{ route('hmo.update-contribution-rate') }}" method="POST" class="space-y-4">
                    @csrf
