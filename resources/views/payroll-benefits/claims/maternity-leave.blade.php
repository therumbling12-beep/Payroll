...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-xl font-extrabold font-outfit text-gray-900">Maternity Leave & Benefit Claims</h1>
            <p class="text-xs text-gray-500 mt-0.5">Statutory maternity leave allowances provided by SSS and Team 2 Workforce Management. Approved non-taxable claims feed directly into Net Pay.</p>
        </div>
        <div class="flex items-center gap-3">
            <span class="flex items-center gap-1.5 text-xs font-semibold text-pink-600 bg-pink-50 border border-pink-200 px-3 py-1.5 rounded-full">
                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="border-b border-gray-100 text-[11px] font-bold text-gray-400 uppercase tracking-wider">
                            <th class="py-3 px-4">Claim Ref</th>
                            <th class="py-3 px-4">Employee</th>
                            <th class="py-3 px-4">Claim Details</th>
                            <th class="py-3 px-4">Benefit Amount (Non-Taxable)</th>
                    ...
                    <tbody class="divide-y divide-gray-50 text-xs">
                        @forelse($claims as $claim)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="py-3.5 px-4 font-extrabold text-pink-600">₱{{ number_format((float)$claim->amount, 2) }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="py-8 text-center text-gray-400 text-xs">No maternity leave benefit claims found.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
        ...
        <div x-show="showModal" x-cloak class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
            <div @click.away="showModal = false" class="bg-white rounded-2xl border border-gray-100 p-6 max-w-lg w-full shadow-2xl space-y-4">
                <div class="flex items-center justify-between border-b border-gray-100 pb-3">
                    <h3 class="text-sm font-bold font-outfit text-gray-900">File Maternity Benefit Claim</h3>
                ...
                <form action="{{ route('claims.store') }}" method="POST" class="space-y-4">
                    @csrf
                    <input type="hidden" name="type" value="maternity">

                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Employee</label>
                    ...
                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Benefit Allowance Amount (₱)</label>
                        <input type="number" step="0.01" name="amount" required placeholder="e.g. 45000.00" class="w-full text-xs bg-gray-50 border border-gray-200 rounded-xl px-3.5 py-2 text-gray-800 focus:outline-none focus:border-[#F44336]">
                    </div>

                    <div>
                        <label class="block text-xs font-bold text-gray-700 mb-1">Claim Details / Statutory Duration</label>
                        <textarea name="description" rows="3" required placeholder="105-Day Statutory Maternity Benefit Advance..." class="w-full text-xs bg-gray-50 border border-gray-200 rounded-xl p-3 text-gray-800 focus:outline-none focus:border-[#F44336]"></textarea>
                    </div>

                    <div class="flex items-center justify-end gap-3 pt-3 border-t border-gray-100">
                        <button type="button" @click="showModal = false" class="text-xs font-bold text-gray-500 px-4 py-2">Cancel</button>
                        <button type="submit" class="bg-[#F44336] hover:bg-[#D32F2F] text-white font-bold text-xs px-5 py-2 rounded-xl">Submit Maternity Claim</button>
                    </div>
                </form>
            </div>
        </div>
